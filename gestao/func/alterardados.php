...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($sem_erro) {
        if ($senha !== "") {
            $stmt = $conexao->prepare("UPDATE funcionarios SET nickname = ?, senha = ?, nome = ?, email = ?, funcao = ? WHERE idFuncionario = ?");
            $stmt->bind_param("sssssi", $nickname, $senha_hash, $nome, $email, $funcao, $id);
        } else {
            $stmt = $conexao->prepare("UPDATE funcionarios SET nickname = ?, nome = ?, email = ?, funcao = ? WHERE idFuncionario = ?");
            $stmt->bind_param("ssssi", $nickname, $nome, $email, $funcao, $id);
        }
...
$stmt = $conexao->prepare("SELECT nome, nickname, email, funcao FROM funcionarios WHERE idFuncionario = ?");
$stmt->bind_param("i", $id);
$stmt->execute();
$stmt->bind_result($nome, $nickname, $email, $funcao);
if (!$stmt->fetch()) {
...
<head>
    <meta charset="UTF-8">
    <title>Alterar Dados | Dueños de Pizza</title>
...
<body>
    <div class="alterar-dados-box">
        ...
        <form method="post" action="alterardados.php?id=<?= $id ?>">
            ...
            <input type="text" name="novoemail" value="<?= $email ?>" placeholder="Email"><br>
            <input type="text" name="novofuncao" value="<?= $funcao ?>" placeholder="Função"><br>
            <button type="submit" class="btn">Alterar</button>
            <a href="procuraltera.php" class="btn">Voltar</a>
        </form>
    </div>
</body>
</html>
